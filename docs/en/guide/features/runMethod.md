...
Click the <ColorIcon icon="runMethod" /> icon on the left side of a Java/Kotlin method, or trigger `RunMethod(FR)` on the method. The plugin opens the `RunMethod` tab and generates parameter editors from the method parameters.
